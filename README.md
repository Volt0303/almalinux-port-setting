...
├─ extras/
│  └─ 99-lan-multiport.conf     同一サブネット運用時の sysctl 設定
├─ install.sh                    かんたん導入（フォント→ビルド→インストールを自動実行）
...
> **既にポート対応表がある機種**（2台目以降）は、下記1コマンドで
> 手順2・4・5 をまとめて実行できます。
> ```bash
> bash install.sh
> ```
> パッケージ導入 → ビルド → インストールを自動実行し、設定の一致状況も確認します。
...
- **GUI**：実行結果は **デスクトップの `result.csv`** に **追記** 保存
  （ロケール自動判定 `デスクトップ`／`Desktop`。sudo実行時も実ユーザ所有に補正）。
- **CLI**：既定は `logs/result.csv`（`--log` で変更可）。
...
| シリアルの候補が出ない | 完全一致を入力済み／該当なし | 一部だけ入力（例「199」）。候補をクリックで選択。↓キーで一覧へ移動可 |
...
## 同一サブネットに複数ポートを設定する場合

複数のLANポートを**同じサブネット**に設定すると、**疎通できるポートが1つだけ**に
なることがあります。IPアドレス自体は正しく設定され照合もOKになるため、
設定作業では気付けません。

**原因**: Linux の `rp_filter`（逆経路フィルタ）が既定で strict(1) のため、
`main` テーブルで採用されなかった経路のポートに届いた応答が破棄されます。

**対処**: `rp_filter` を loose(2) にします。
```bash
sudo cp extras/99-lan-multiport.conf /etc/sysctl.d/
sudo sysctl --system
sysctl net.ipv4.conf.all.rp_filter      # → 2 であること
```
`/etc/sysctl.d/` に置くため**再起動後も維持**されます。
（`ip route add` / `ip rule add` による手動設定は**再起動で消える**ため非推奨）

> 実効値は `all` と各インターフェース設定の **大きい方**が採用されます
> （`all=2` にすれば個別が 1 でも loose になります）。

> **補足**: 上記はポートを個別に指定して通信する場合の対処です。送信元を
> 指定しない通信は依然として1つのポートに集中するため、用途によっては
> ポリシールーティング（NetworkManager の `ipv4.routing-rules`）が必要です。

**検知**: 支給ファイル読込時、同一サブネットのポートがあると警告を表示します。
```
SN xxx: LAN1、LAN2、LAN3 が同一サブネット (192.168.1.0/24) です。...
```

> ポートごとにサブネットが分かれている場合、本対応は不要です。
